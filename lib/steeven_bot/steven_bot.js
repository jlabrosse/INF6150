...
var commands = {
    ...
    "lister": {
		...
		process: function(bot, msg, suffix) {
            console.log(formateur.creerMessage("afficherTypeRarete", suffix).ecrire());
            //if(suffix)
			//	msg.author.sendMessage( "!lister ne prends pas d'argument");
			//else {
			//	msg.channel.sendMessage(JSON.stringify(gw.obtenirMapType(), null, 2) + '\n'
    ...
    "stocks": {
        description: "Affiche les chutes de stocks",
        process: function(bot, msg, suffix) {
            msg.channel.sendMessage(msg.author + formateur.creerMessage("AfficherStocks", suffix).ecrire());
            if(suffix){
                msg.channel.sendMessage( "!stocks ne prend pas d'arguments.");
            }
        }
    }
};
