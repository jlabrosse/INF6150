	...
	var commands = {	
		...
		"montre_moi": {	//TODO: envoyer en promise
			...
			process: function(bot, msg, suffix) {
				if(!suffix){
					...
					msg.author.sendMessage(JSON.stringify(gw.obtenirMapType(), null, 2));
				} else if(suffix == 'rarete') {
					msg.author.sendMessage(JSON.stringify(gw.obtenirMapRarete(), null, 2));
				} else {
					msg.author.sendMessage( "Argument(s) incorrect(s)");
				}
			}
		}
	};
