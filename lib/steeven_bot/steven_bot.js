...
var commands = {
    ...
    "notifiez_moi": {
			...
			process: function(bot, msg, suffix) {
					fabrique.construireUtilisateur(msg.author.id, suffix);
					msg.author.sendMessage( msg.author + formateur.factory().creerMessage("abonnementItem", suffix).ecrire() );
					if(!suffix){
    ...
    "notifiez_canal": {
        ...
        process: function(bot, msg, suffix) {
            fabrique.construireCanal(msg.guild.id, msg.channel.id, suffix);
            msg.channel.sendMessage(msg.author + formateur.factory().creerMessage("abonnementItem", suffix).ecrire() );
            if (!suffix) {
    ...
    "lister_types": {
		...
		process: function(bot, msg, suffix) {
            if(suffix)
              msg.author.sendMessage( "!lister_types ne prends pas d'argument");
            else {
              msg.channel.sendMessage(formateur.creerMessage("afficherType", suffix).ecrire());
            }
		}
	},
    "lister_raretes": {
        ...
        process: function(bot, msg, suffix) {
            if(suffix)
              msg.author.sendMessage( "!lister_raretes ne prends pas d'argument");
            else {
              msg.channel.sendMessage(formateur.creerMessage("afficherRarete", suffix).ecrire());
            }
        }
    },
	"meilleurs_profits": {
		description: "Affiche les 10 meilleurs profits",
		process: function(bot, msg, suffix) {
			msg.channel.sendMessage(msg.author + formateur.creerMessage("meilleursProfits", suffix).ecrire());
			if(suffix){
				msg.channel.sendMessage( "!meilleures_offres ne prend pas d'arguments.");
			}
		}
	},
    "pertes": {
        description: "Affiche les 5 pires pertes",
        process: function(bot, msg, suffix) {
            msg.channel.sendMessage(msg.author + formateur.creerMessage("afficherPertes", suffix).ecrire());
            if(suffix){
                msg.channel.sendMessage( "!perte ne prend pas d'arguments.");
            }
        }
    },
    "surveiller_stock": {
        description: "Mets le pointeur des messages de surveillance sur ce canal",
        process: function(bot, msg, suffix) {
            fabrique.construireSurveillance(msg.channel.id);
            msg.channel.sendMessage(msg.author + "la surveillance sera affichee sur ce canal");
            if(suffix){
                msg.channel.sendMessage( "!perte ne prend pas d'arguments.");
            }
        }
...
module.exports.envoyerMessageChannel = function(destinataire, channel, msg) {
	bot.channels.get(channel).sendMessage(destinataire + msg);
}
module.exports.envoyerMessagePerso = function (destinataire, msg) {
    bot.users.get(destinataire).sendMessage(msg);
}