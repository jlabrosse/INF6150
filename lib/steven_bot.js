	...
	try {
		...
		console.log("Verifer que 'npm install' ne provoque pas d'erreur.");
		process.exit();
	}
	console.log("Demarrage de Steven.\nVersion de Node: " + process.version + "\nVersion de Discord.js: " + Discord.version);
	...
	try {
		...
		console.log("Fichier auth.json inexistant ou invalide, un champ bot_token est obligatoire.\n"+e.stack);
		process.exit();
	}
	...
	try{
		...
		try{
			if(fs.lstatSync("./config.json").isFile()){
				console.log("ATTENTION: config.json est impossible a lire!\n" + e.stack);
			}
		} catch(e2){
	...
	bot.on("ready", function () {
		console.log("En ligne! Disponible dans  " + bot.guilds.array().length + " serveur(s)");
		console.log("Taper "+Config.commandPrefix+"aide sur Discord pour une liste des commandes");
	...
	function verifierMessage(msg, isEdit) {
		...
		if(msg.author.id != bot.user.id && (msg.content.startsWith(Config.commandPrefix))){
			console.log("Traite " + msg.content + " de " + msg.author + " comme une commande");
	...
	if(AuthDetails.bot_token){
		console.log("Identification avec bot_token.");
		bot.login(AuthDetails.bot_token);
	} else {
		console.log("Le bot support seulement des identifications a partir d'un token Discord. (Discord interdit les comptes utilisateurs pour un bot)");
	}