...
function construireDonneesGW2(donnees) {
    ...
    for(var i = 0; i < donnees.results.length; i++){
        //console.log(donnees.results[i].max_offer_unit_price+" offer");
        ...
        console.log(steevenData[donnees.results[i].name].prix_offre_max + " offer");
    }
...
module.exports = {
    ...
    mapType: function(callback){
        aspirateur.getmapType(function(resultat){
            callback(resultat);
        });
    },
    mapRarity: function(callback){
        aspirateur.getmapRarity(function(resultat){
            callback(resultat);
        });
    }
