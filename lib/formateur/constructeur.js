...
function Factory() {
	...
	this.creerMessage = function (type, nomItem) {
		...
		if(type == "meilleursProfits"){
			...
			message = new Pertes();
		} else if(type === "afficherTypeRarete"){
			message = new AfficherTypeRarete(); 
		} else if(type === "AfficherStocks"){
			message = new Stocks(); 	
		} else if(item == null) {
			message = new Erreur();
...
var AfficherTypeRarete = function(){
	...
	for(var i = 0; i < types.results.length; i++){
        console.log(JSON.stringify(types.results[i], null, 2));
    }
